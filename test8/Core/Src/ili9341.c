...
uint16_t tft_W = ILI9341_WIDTH;
uint16_t tft_H = ILI9341_HEIGHT;
...
void ILI9341_Fill_Rect(unsigned int x,unsigned int y, unsigned int w,unsigned int h, uint16_t color) {
...
//    if((y + h - 1) >= ILI9341_HEIGHT) h = ILI9341_HEIGHT - y;
	...
	ILI9341_SetCursorPosition(x, y, x+w-1, y+h-1);
    for(y = h; y > 0; y--) {
        for(x = w; x > 0; x--) {
      ILI9341_SendData(color>>8);
				ILI9341_SendData(color&0xff);
...
void drawColorBar(int value,int spotx, int spoty, int pix){
	...
	ILI9341_Fill_Rect(spotx+(i*pix),spoty,spotx+(i*pix)+pix/2,spoty+18, COLOR_MAGENTA);
	}
}
...
int RGB565CONVERT(int r,int g,int b) {
    long RGB565;
...
void display_meter(float scale){
 ...
 for (float i = start_angle; i < end_angle; i = i + 0.01)
  {
	 ILI9341_DrawPixel(x + cos(i) * r, y + sin(i) * r, COLOR_WHITE); // center point is (x,y)
	 ILI9341_DrawPixel(x + cos(i) * r * 1.01, y + sin(i) * r * 1.01, COLOR_WHITE);
  }
 for (float i = start_angle; i < end_angle; i = i + (end_angle-start_angle)*major_graduations)
  {
	 ILI9341_drawLine(x + cos(i) * r, y + sin(i) * r, x + cos(i) * r * 1.1, y + sin(i) * r * 1.1, COLOR_WHITE);

//	snprintf(buff, sizeof(buff), "x=%d", scale);
//	ILI9341_printText(buff, x + cos(i) * r * 1.16-3,  y + sin(i) * r * 1.16, COLOR_WHITE, COLOR_BLUE, 2);

    scale++;
  }
 ILI9341_drawCircle(x,y,5, COLOR_GREEN); // Draw a small circel around the meter needle hinge point
// ILI9341_Fill_Rect(2,22, 310, 200, COLOR_GREEN); // Draw the outer rectangle
}
...
void arrow(int x1, int y1, int x2, int y2, int alength, int awidth, int colour) {
  ...
  ILI9341_drawLine(x1, y1, dx, dy, colour);
  ILI9341_drawLine(x3, y3, x4, y4, colour);
  ILI9341_drawLine(x3, y3, x2, y2, colour);
  ILI9341_drawLine(x2, y2, x4, y4, colour);
}
...
void display_value (float value){
  ...
  if (last_value != value) {
    arrow(x,y,x2, y2, 10, 10, COLOR_BLACK);   // Erase arrow in old position
    ILI9341_drawLine(x, y, x2, y2, COLOR_BLACK);
  }
  ...
  ILI9341_drawLine(x, y, x2, y2, COLOR_GREEN);
  arrow(x, y, x2, y2, 10, 10, COLOR_GREEN);    // Draw arrow in new position
  last_value = value;
  ILI9341_drawCircle(x,y,5, COLOR_GREEN);
}
...
void TFT_Draw_Line(int16_t x1, int16_t y1, int16_t x2, int16_t y2, uint16_t color) {
	...
	if (y1 == y2) {
		if (x2 > x1) {
			ILI9341_drawFastHLine(x1, y1, x2 - x1 + 1, color);
		} else {
			ILI9341_drawFastHLine(x2, y1, x1 - x2 + 1, color);
		}
		return;
	} else if (x1 == x2) {
		if (y2 > y1) {
			ILI9341_drawFastVLine(x1, y1, y2 - y1 + 1, color);
		} else {
			ILI9341_drawFastVLine(x1, y2, y1 - y2 + 1, color);
		}
		return;
	}
